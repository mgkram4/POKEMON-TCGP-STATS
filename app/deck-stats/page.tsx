...
// Updated Pokemon API helper functions
...
const DeckItem = ({ deck, showStat = "games" }: { deck: DeckStats, showStat?: "games" | "winRate" | "matchups" }) => {
  ...
  return (
    <motion.div
      ...
      className="flex justify-between items-center p-2 rounded-lg hover:bg-white/30 transition-colors"
    >
    ...
    const loadDeckStats = async () => {
      try {
        ...
        if (data && data.tiers) {
          ...
          setDeckStats(statsArray.sort((a, b) => b.totalGames - a.totalGames));
        }
        setIsLoading(false);
      } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <motion.div 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-2xl text-gray-600 flex items-center gap-3"
        >
